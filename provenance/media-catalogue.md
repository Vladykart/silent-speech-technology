...
| Demo signal console | Eight-channel arrays deterministically synthesized from original authored patterns in `demo/core.js`; SVG paths and candidate meters generated locally by `demo/script.js` | Original HTML/CSS/SVG/fixture behavior; not biological, copied upstream code, or model output. | Synthetic normalized values; not biometric, voltage/time calibrated, inferred, or measured performance. |
| Recorded-replay traces/features/candidates | Generated at runtime from locally fetched official sEMG arrays and official released weights; drawn with project-authored HTML/CSS/JS; no retained screenshot/media | Underlying Silent Speech EMG v1.0 data/model assets are David Gaddy releases under CC BY 4.0; exact DOIs/hashes in `realtime/assets-manifest.json`. Assets/output screenshots are not committed. | Replay of selected single-speaker records through a real model; not live capture, project accuracy, representative evaluation or calibrated probability. |
| `realtime/assets-manifest.json` | Project-authored machine-readable locator/provenance record | Contains only official URLs, identifiers, hashes, sizes and selected archive-member identities—no checkpoint, array, audio or biometric payload. | The referenced 207 MB checkpoint and 3.92 GB dataset remain ignored local assets and may not be inferred as owned project IP. |
