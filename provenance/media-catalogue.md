...
| Static demo waveform | Arrays authored in `demo/core.js`; SVG paths generated locally by `demo/script.js` | Original illustrative data; not biological or model output. | Normalized, not time/voltage calibrated, not measured accuracy. |
| Investor real-replay runtime displays | Project-authored canvas/HTML views generated only from the active frozen official recording and one official released checkpoint: 8-channel source envelopes, source/filtered comparison, 112-feature and 80-bin heatmaps, 48-class top-output inspection, candidates and timeline. No retained screenshot/media. | Underlying Silent Speech EMG v1.0 data/model assets are David Gaddy releases under CC BY 4.0; authority and changes are recorded in `realtime/assets-manifest.json`, `realtime/sample-manifest.json`, and `provenance/realtime-assets.md`. Browser derivatives are bounded min/max-decimated, robustly centered/scaled, time-binned, normalized and/or signed-8-bit quantized; no full array/tensor, audio, model, download, or display scale is exposed. | Official single-speaker recorded replay through one real released model; not live capture, untouched raw data, audio/ground truth, project accuracy, representative evaluation or calibrated probability. Evidence is volatile and screenshots/model outputs are not committed. |
| `realtime/assets-manifest.json`, `realtime/sample-manifest.json`, `realtime/model-registry.json` | Project-authored machine-readable locator/provenance/selection records | Contain official URLs, identifiers, hashes, sizes, frozen member identities/prompts and status metadata—no checkpoint, array, audio or biometric payload. They are not served to the browser; the public API emits safe views only. | Referenced 216.9 MB checkpoint and 3.92 GB dataset remain ignored owner-controlled assets and may not be inferred as owned project IP. The curated set is not an accuracy sample. |
...
| `pitch/quiet-channel-4-slide-deck.pptx` | Deterministic standard-library OOXML package built from `pitch/deck-4-slide.md` by `pitch/build_4_slide_pptx.py` | Contains only native project-authored DrawingML text, shapes, connectors and chart primitives; no template, image, logo or third-party media. Hash evidence: `pitch/quiet-channel-4-slide-build.json`. | Representative geometry-matched SVGs were rendered with headless Chromium; exact PowerPoint opening/rendering remains unverified. Artifact is not evidence of hardware or performance. |
| Deck/demo UI | HTML/CSS/JS authored for this repository | TWPS commit `e3ce5a9` was inspected only for interaction/file-spirit reference; no TWPS identity, claims, media, CSS or branding is included. | Representative four-slide geometry QA completed; 13-slide browser surfaces and exact PowerPoint still require rehearsal. |
...
No third-party paper PDF, chart, screenshot, dataset, sample, model or weight is vendored. `realtime/fetch_assets.py` locates/downloads official CC BY 4.0 data/model files only into git-ignored local storage; the adapted MIT architecture code carries its notice. References and exact revisions appear in [`../research/references.md`](../research/references.md). Licences described there apply to cited artifacts; they do not automatically license screenshots, trademarks, paper figures, other participant data, dependencies, endorsement, or this project.
