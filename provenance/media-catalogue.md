...
| Real-model bench traces/features/candidates | Generated at runtime by project-authored `realtime/app/` and drawn with local HTML/CSS/JS; no retained screenshot/media | Raw signal is synthetic with injected noise; candidates are genuine outputs of the project-generated trained weights. No participant or upstream sample/model output. | Synthetic-regime executable behavior only; not physical sensing, measured accuracy or probability calibration. |
| `realtime/models/silent_ctc_v1.pt` + manifest | Deterministically trained by `realtime/train.py` on project-generated multi-channel signals | Project-authored learned parameter artifact; no human/biometric data or external checkpoint. SHA-256 and provenance in adjacent JSON. | Not a media asset and not evidence of real-EMG/model performance; included here for complete local-asset provenance. |
