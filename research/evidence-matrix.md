...
| E05 / R05, R16 | repository + model registry | sEMG synthesis/recognition code and pretrained artifact | exact commit; dependency stack names DeepSpeech 0.7 | README claims latest approx. 36% ASR-based WER; not used as peer-reviewed core result | code MIT; model record CC BY 4.0; data terms separate; no execution/download | a reproducibility starting point and licence-separation lesson | turnkey offline demo, maintained product, commercial rights bundle |
...
- This project has **no** WER, command accuracy, false accept/reject rate, latency, calibration, participant, comfort, power, or hardware number.
