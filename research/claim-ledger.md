...
**Release:** updated 2026-08-11. `C` IDs appear in the deck notes. **Green** means safe only with the wording/conditions shown. **Amber** is a hypothesis or external result requiring its qualifier. **Red** is prohibited for this project. `H` rows are captain holds and must not be silently decided.
...
| C34 | Green | The production acquisition path read-only replays checksum-bound official single-speaker 1 kHz, eight-channel native-`float64` sEMG from Silent Speech EMG v1.0; it is not live capture. Runtime/model conversion and every browser display transformation are explicit. | Dataset Zenodo 4064409 (concept 4064408), CC BY 4.0. Browser envelopes are decimated/centered/scaled/quantized and never “untouched raw.” No production simulator/fallback, participant collection, device, electrical-safety review, calibration or cross-speaker evidence. |
...
| C36 | Green | A private Tailnet-only research demo delivery path exists; it is not public and not a product/customer deployment. Bounded transformed run evidence is sent to its authorized browser. | Stable route/service ownership is deployment evidence, not privacy, security, reliability, adoption or product evidence. No third-party telemetry/persistence/raw archive/actuation; “no egress” is prohibited because browser transfer occurs. |
...
| C22 | Green | The project has no measured project silent-speech accuracy, WER, end-to-end hardware latency, power, calibration, participant/cohort, comfort or hardware number. | `realtime/` may expose separately timed model-forward and project-decoder software diagnostics for one active run only. Both exclude recording duration, sensing, hardware, network, endpointing, confirmation and output. “Single speaker” is attributed source scope, not a project participant count. |
