...
PPTX = ROOT / "quiet-channel-evidence-deck.pptx"
EVIDENCE = ROOT / "pptx-build.json"
BUILDER = ROOT / "build_pptx.py"
...
REQUIRED_TEXT = [
    "A quiet path into the assistant you already run.",
    "e& frontline retail advisors and field technicians.",
    "CONCEPT / SIMULATED PIPELINE",
    "77.5±6.6%", "59.3±2.2%", "preprint", "n=4", "8 commands + rest",
    "AED 1,000,000", "per-seat annual", "acquisition/exit", "TBD",
    "EarCommand", "EchoSpeech", "MuteIt", "Whispp", "R38–R41",
    "Go. Change. Or stop on purpose.", "CLAIM LEDGER",
]
REQUIRED_MEDIA = {
    "ppt/media/concept-research-rig.svg",
    "ppt/media/pipeline-gates.svg",
    "ppt/media/evidence-gap.svg",
    "ppt/media/privacy-flow.svg",
}
...
def main() -> int:
    ...
    try:
        with ZipFile(args.pptx) as zf:
            ...
            expected_slides = [f"ppt/slides/slide{i}.xml" for i in range(1, 14)]
            if ordered_slides != expected_slides: errors.append(f"slide count/order mismatch: {ordered_slides}")
            expected_notes = [f"ppt/notesSlides/notesSlide{i}.xml" for i in range(1, 14)]
            if [n for n in expected_notes if n in name_set] != expected_notes: errors.append("must contain 13 ordered notes slides")
            slide_texts = []
            editable_shapes = 0
            ...
            if editable_shapes < 100: errors.append(f"too few editable DrawingML shapes: {editable_shapes}")
            ...
            if not REQUIRED_MEDIA.issubset(name_set): errors.append("not all four original SVGs are embedded")
    ...
    if not args.evidence.is_file():
        ...
        if evidence.get("slides") != 13 or evidence.get("notes_slides") != 13: errors.append("build evidence slide counts wrong")
    ...
    print(f"PPTX validation passed: 13 slides, 13 notes, editable shapes, 4 original SVGs, valid relationships, no remote relationships, deterministic sha256={digest(args.pptx)}")
    return 0
