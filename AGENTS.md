...
- Quick validation: `python3 tools/validate_project.py && python3 demo/generate_manifest.py && python3 demo/validate.py && node demo/tests/core.test.js && python3 demo/tests/responsive_layout.test.py && python3 lab/validate.py && python3 -m unittest lab/tests/test_lab.py && python3 pitch/validate.py && python3 pitch/validate_pptx.py && python3 pitch/validate_4_slide_pptx.py && python3 realtime/validate.py`; after local dependencies exist, add `realtime/.venv/bin/python -m unittest discover -s realtime/tests -v`.
...
- Keep `demo/` as the locked authored-fixture/no-model experience. Its artifact/deployment authorities are [`demo/artifact-manifest.json`](demo/artifact-manifest.json) and [`demo/DEPLOYMENT.md`](demo/DEPLOYMENT.md). Separately, `realtime/` replays official CC BY 4.0 single-speaker sEMG through official released weights; run `realtime/fetch_assets.py`, keep all model/data/replay files under ignored `local_assets/`, and treat selected outputs as neither live hardware nor accuracy evidence.
